...
function my_calendar_invoke( $atts ){

    if(!is_user_logged_in()) return;
    
    global $wpdb;
    $current_user = wp_get_current_user();
    $result = $wpdb->get_results( "SELECT * FROM `wp_cpappbk_messages` WHERE notifyto='".$current_user->user_email ."' ");
    $html = "<ul>";

    foreach ($result as $rkey => $item) {
...
add_shortcode( 'my_calendar', 'my_calendar_invoke' );
...
function custom_add_google_fonts() {
    wp_enqueue_style( 'custom-google-fonts', 'https://fonts.googleapis.com/css?family=Raleway:400,700,700i,800|Ubuntu:400,700', false );
}
